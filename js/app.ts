const Storage = {
  ...
  getOptions() {
    return Storage.load("options").then(function(data) {
      ...
      const options = _.clone(Storage.options.defaults);
...
const Weather = {
  ...
  parse(data) {
    return Storage.getOption("unitType").then(function(unitType) {
      ...
      for (const i = Weather.$el.forecast.length - 1; i >= 0; i--) {
  ...
  noLocation() {
    ...
    $("#set-location").submit(() => {
      ...
      if (!_.isEmpty(address)) {
        // Geocode address
        Location.gecodeAddress(address).then((data)=> {
          const options = {};
...
$(".options").click(() => {
  Storage.getOptions().done((options) => {
